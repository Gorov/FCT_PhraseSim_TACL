...
int EmbeddingModel::LoadLM(string modelname)
{
    ...
    syn1neg = (real *)malloc(words * size * sizeof(real));
    real len;
    ...
    for (b = 0; b < words; b++) {
        ...
        for (a = 0; a < size; a++) {
            fread(&syn0[a + b * size], sizeof(real), 1, f);
        }
//        len = 0.0;
...
int EmbeddingModel::LoadHSLM(string modelname)
{
    long long words, size, a, b;
    char ch;
    FILE *f = fopen(modelname.c_str(), "r");
    if (f == NULL) {
        printf("Input file not found\n");
        return -1;
    }
    ...
    layer1_size = size;
    syn0 = (real *)malloc(words * size * sizeof(real));
    syn1 = (real *)malloc(words * size * sizeof(real));
    
    if (syn0 == NULL || syn1 == NULL) {
        printf("Cannot allocate memory: %lld MB\n", words * size * sizeof(float) / 1048576);
        return -1;
    }
    ...
    for (b = 0; b < words; b++) {
        ...
        vocablist.push_back(tmpword);
        for (a = 0; a < size; a++) fread(&syn0[a + b * size], sizeof(real), 1, f);
        //for (a = 0; a < size; a++) fscanf(f, "%f", &syn0[a + b * size]);
    }
    for (b = 0; b < words - 1; b++) {
        fscanf(f, "%c", &ch);
        for (a = 0; a < size; a++) fread(&syn1[a + b * size], sizeof(real), 1, f);
        //for (a = 0; a < size; a++) fscanf(f, "%f", &syn1[a + b * size]);
    }
...
int EmbeddingModel::LoadHalfLM(string modelname)
{
    long long words, size, a, b;
    char ch;
    real len;
    ...
    for (b = 0; b < words; b++) {
        ...
        len = sqrt(len);
        for (a = 0; a < size; a++) syn0[a + b * size] /= len;
        for (a = 0; a < size; a++) syn1neg[a + b * size] = syn0[a + b * size];
        //for (a = 0; a < size; a++) syn1neg[a + b * size] = 0.0;
    }
    fclose(f);
    return 0;
}
...
int EmbeddingModel::LoadLMAdditively(string modelname)
{
    long long words, size, a, b;
    char ch;
    long long l2;
    ...
    for (b = 0; b < words; b++) {
        ...
        float tmp;
        if (iter == vocabdict.end()) {
            for (a = 0; a < size; a++) fread(&tmp, sizeof(real), 1, f);
            for (a = 0; a < size; a++) fread(&tmp, sizeof(real), 1, f);
        }
        else {
            l2 = iter -> second * layer1_size;
            for (a = 0; a < size; a++) fread(&syn0[a + l2], sizeof(real), 1, f);
            for (a = 0; a < size; a++) fread(&syn1neg[a + l2], sizeof(real), 1, f);
        }
    }
    fclose(f);
    return 0;
